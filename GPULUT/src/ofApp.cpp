...
void ofApp::setup() {
	...
	/*videop.load("video/mount.mo);*/
	videop.load("video/landscape.mp4");
	videop.setVolume(0);
	videop.play();
...
void ofApp::update() {
	...
	video.update();
	videop.update();
	if (videop.isFrameNew()) {
		videot.loadData(videop.getPixels());
	}
...
void ofApp::draw() {
	...
	if (isThumbnailView) {
		for (int i = 0; i < 9; i++) {
			...
			if (videot.isAllocated()) {
				videot.draw(thumbnailWidth * fmodf(i, 3), thumbnailHeight * floor(i / 3), thumbnailWidth, thumbnailHeight);
			}
